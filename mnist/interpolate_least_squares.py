...
if __name__ == '__main__':
    ...
    out_dir = '/u/grewalka/lasagne/gamma-experiment/least-squares/50/'# % (params['discriminator_iters'])
        ...
        for epoch in range(params['epochs']):
            ...
            for gamma in (np.array(range(-50, 150)) / 100.):
                ...
                for n in range(32):
                    ...
                    print(D_grad_norm_value(x_hat).shape)
            ...
            with open(os.path.join(out_dir, 'D_samples_gamma_star_%d.npz' % (epoch+1)), 'w+') as f:
                np.savez(f, D_samples)
            with open(os.path.join(out_dir, 'D_grad_norms_gamma_star_%d.npz' % (epoch+1)), 'w+') as f:
                np.savez(f, D_grad_norms)
            
            with open(os.path.join(out_dir, 'G_loss_gamma_star_%d.npz' % (epoch+1)), 'w+') as f:
                np.savez(f, G_losses)
            with open(os.path.join(out_dir, 'G_loss_grad_norms_gamma_star_%d.npz' % (epoch+1)), 'w+') as f:
