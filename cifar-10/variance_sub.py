...
"""
VRAL while subtracting the mean of the fake samples on CIFAR-10.
"""
...
if __name__ == '__main__':
    if BN_D and BN_G:
        out_dir = '/u/grewalka/cifar10/%d/variance_sub/%d_%d_%d_1/' % (DIM_H, ITERS_F, ITERS_R, ITERS_D)
    elif BN_D:
        out_dir = '/u/grewalka/cifar10/%d/variance_sub/bn_d/%d_%d_%d_1/' % (DIM_H, ITERS_F, ITERS_R, ITERS_D)
    elif BN_G:
        out_dir = '/u/grewalka/cifar10/%d/variance_sub/bn_g/%d_%d_%d_1/' % (DIM_H, ITERS_F, ITERS_R, ITERS_D)
    else:
        out_dir = '/u/grewalka/cifar10/%d/variance_sub/nbn/%d_%d_%d_1/' % (DIM_H, ITERS_F, ITERS_R, ITERS_D)
    ...
    r_real = get_output(R, y_1)
    r_fake = get_output(R)
    f_real = get_output(F, y_0)
    f_fake = get_output(F)

    # Loss functions
    F_loss = (f_fake.mean() - f_fake - T.nnet.softplus(-f_real) - T.nnet.softplus(f_fake.mean() - f_fake)).mean()
    R_loss = (-r_fake - T.nnet.softplus(-r_fake) - T.nnet.softplus(-r_real)).mean()
    D_loss = ((f_fake - f_fake.mean())**2).mean() + (r_fake**2).mean() - y_real.mean() + y_fake.mean()
    ...
    train_F = function([y_0, z], outputs=F_loss, updates=updates_F, allow_input_downcast=True)
    train_R = function([y_1, X], outputs=R_loss, updates=updates_R, allow_input_downcast=True)
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(num_examples / BATCH_SIZE):
            ...
            for k in range(ITERS_D):
                ...
                y_0_i = np.float32(np.random.normal(loc=FAKE_MEAN,size=(BATCH_SIZE,1)))
                z_i = np.float32(np.random.normal(size=(BATCH_SIZE,DIM_Z)))
                train_F(y_0_i, z_i)
                
                # Train real data discriminator
                y_1_i = np.float32(np.random.normal(loc=REAL_MEAN,size=(BATCH_SIZE,1)))
                x_i = iterator.next()[0]
                train_R(y_1_i, x_i)
