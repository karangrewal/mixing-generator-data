...
if __name__ == '__main__':
    ...
    out_dir = '/u/grewalka/cifar10/gamma-experiment/variance/%d_%d_%d_1/' % (params['iters_F'], params['iters_R'], params['iters_D'])
    
    X = T.tensor4()
    z = T.fmatrix()

    D, G = discriminator(X, use_batch_norm=True), generator(z)
    ...
    # D_loss = -(T.nnet.softplus(-f_fake) + T.nnet.softplus(-r_fake) + f_fake + r_fake).mean() # Regular Loss
    D_loss = (T.nnet.softplus(-f_fake) + T.nnet.softplus(-r_fake)).mean() # Proxy Loss
