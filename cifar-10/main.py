...
""" Regular and Proxy GAN for CIFAR-10. """
...
if __name__ == '__main__':
    if METHOD == 'proxy':
        if BN_D and BN_G:
            out_dir = '/u/grewalka/cifar10/proxy/%d_1/' % (ITERS_D)
        elif BN_D:
            out_dir = '/u/grewalka/cifar10/proxy/bn_d/%d_1/' % (ITERS_D)
        elif BN_G:
            out_dir = '/u/grewalka/cifar10/proxy/bn_g/%d_1/' % (ITERS_D)
        else:
            out_dir = '/u/grewalka/cifar10/proxy/nbn/%d_1/' % (ITERS_D)
    elif METHOD == 'regular':
        if BN_D and BN_G:
            out_dir = '/u/grewalka/cifar10/regular/%d_1/' % (ITERS_D)
        elif BN_D:
            out_dir = '/u/grewalka/cifar10/regular/bn_d/%d_1/' % (ITERS_D)
        elif BN_G:
            out_dir = '/u/grewalka/cifar10/regular/bn_g/%d_1/' % (ITERS_D)
        else:
            out_dir = '/u/grewalka/cifar10/regular/nbn/%d_1/' % (ITERS_D)
    else:
        exit(0)
    ...
    for epoch in range(EPOCHS):
        ...
        for i in range(num_examples / BATCH_SIZE):
            ...
            for k in range(ITERS_D):
                x_i = iterator.next()[0]
                z_i = np.float32(np.random.normal(size=(BATCH_SIZE,DIM_Z)))# CHANGE TO DIMZ
                train_D(x_i, z_i)
